...
export const pendingStatus =  createAsyncThunk(
    ...
    try {
        ...
        const {data} = await axios.put(`http://localhost:5000/pending/${arg}`)
        return data;
    } catch (error) {
        isRejectedWithValue(error.response.data)
    }
...
export const approveStatus =  createAsyncThunk(
    ...
    try {
        ...
        const {data} = await axios.put(`http://localhost:5000/approve/${arg}`)
        return data;
    } catch (error) {
        isRejectedWithValue(error.response.data)
    }
...
export const declineStatus =  createAsyncThunk(
    ...
    try {
        ...
        const {data} = await axios.put(`http://localhost:5000/decline/${arg}`)
        return data;
    } catch (error) {
        isRejectedWithValue(error.response.data)
    }
