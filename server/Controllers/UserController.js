...
module.exports.applicationRegister = async (req, res, next) => {
  
    console.log(req.body,"application backend");
    try {
        const applicationData = req.body
        applicationData.userId = req.params.id
        const user = await ApplicationModel.create({ ...applicationData});
...
module.exports.check = async (req, res) => {
    ...
    try {
         ...
         const user = await ApplicationModel.find({userId:id});
         ...
         console.log(err);
        // const errors = handleErrors(err);
         res.status(500).json({ errors, status: false });
        //res.json({ errors, created: false });
    }
}
